...
import apiClient, { clearTokens, getAccessToken, getRefreshToken, setTokens } from '@/lib/apiClient';
...
const setSupabaseSession = async (accessToken?: string | null, refreshToken?: string | null) => {
  if (!accessToken || !refreshToken) return;
  try {
    await supabase.auth.setSession({ access_token: accessToken, refresh_token: refreshToken });
  } catch (error) {
    console.error('Supabase session set failed:', error);
  }
};
