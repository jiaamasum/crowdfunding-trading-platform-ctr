...
export default function DeveloperProjects() {
  ...
  const canEdit = (status: ProjectStatus) => {
    return ['DRAFT', 'NEEDS_CHANGES', 'REJECTED', 'APPROVED'].includes(status);
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold">
            {isArchivedView ? 'Archived Projects' : 'My Projects'}
          </h1>
          <p className="text-muted-foreground mt-1">
            {isArchivedView ? 'Review your archived projects' : 'Manage and track all your projects'}
          </p>
        </div>
        {!isArchivedView && (
