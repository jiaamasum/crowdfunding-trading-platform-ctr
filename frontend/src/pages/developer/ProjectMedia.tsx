...
export default function ProjectMediaPage() {
  ...
  const uploadFile = async (
    ...
    // Simulate progress for now (backend proxy doesn't provide upload progress)
    let progress = 0;
    ...
    try {
      ...
      if (uploadedImages.length > 0 || modelUrl) {
        ...
        if (project?.status === 'APPROVED') {
          ...
          toast.success('Edit request submitted for media updates');
        } else {
          const updated = await projectsApi.update(String(id), payload);
          setProject(updated);
