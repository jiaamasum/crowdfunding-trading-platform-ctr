...
export default function EditProjectPage() {
  ...
  const canEdit = ['DRAFT', 'NEEDS_CHANGES', 'REJECTED'].includes(project.status);
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-col">
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar mode="single" selected={field.value} onSelect={field.onChange} disabled={(date) => date < (form.watch('startDate') || new Date())} initialFocus className="pointer-events-auto" />
                          </PopoverContent>
                        </Popover>
                        <FormMessage />
                      </FormItem>
