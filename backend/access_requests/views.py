...
class AccessRequestListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        access_request = self.perform_create(serializer)
    ...
    def perform_create(self, serializer):
        if getattr(self.request.user, 'is_banned', False):
            raise PermissionDenied('Banned users cannot request access')
        ...
        AuditLog.objects.create(
            action_type=AuditLog.ActionType.ACCESS_REQUEST_CREATED,
            actor=self.request.user,
            target_type=AuditLog.TargetType.ACCESS_REQUEST,
            target_id=str(access_request.id),
            metadata={
                'status': access_request.status,
                'project_id': str(access_request.project.id),
                'project_name': access_request.project.title,
                'investor_id': str(access_request.investor_id),
                'investor_name': access_request.investor.name,
                'investor_email': access_request.investor.email,
            },
        )
        ProjectLedgerEntry.objects.create(
            project=access_request.project,
            entry_type=ProjectLedgerEntry.EntryType.ACCESS_REQUEST_CREATED,
            actor=self.request.user,
            metadata={
...
class AccessRequestRevokeView(APIView):
    """Investor revokes an access request."""
    permission_classes = [permissions.IsAuthenticated]

    def post(self, request, pk):
        try:
            access_request = AccessRequest.objects.get(pk=pk, investor=request.user)
        ...
        if access_request.status != AccessRequest.Status.PENDING:
            return Response({'error': 'Access request cannot be revoked'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        AuditLog.objects.create(
            action_type=AuditLog.ActionType.ACCESS_REQUEST_REVOKED,
            actor=request.user,
            target_type=AuditLog.TargetType.ACCESS_REQUEST,
            target_id=str(access_request.id),
            metadata={
        ...
        ProjectLedgerEntry.objects.create(
            project=access_request.project,
            entry_type=ProjectLedgerEntry.EntryType.ACCESS_REQUEST_REVOKED,
            actor=request.user,
            metadata={
                'access_request_id': str(access_request.id),
                'status': access_request.status,
...
class AccessRequestDecideView(APIView):
    ...
    def post(self, request, pk):
        ...
        ledger_type = {
            'APPROVED': ProjectLedgerEntry.EntryType.ACCESS_REQUEST_APPROVED,
            'REJECTED': ProjectLedgerEntry.EntryType.ACCESS_REQUEST_REJECTED,
            'REVOKED': ProjectLedgerEntry.EntryType.ACCESS_REQUEST_REVOKED,
        }[access_request.status]
        ProjectLedgerEntry.objects.create(
            project=access_request.project,
